...
- Tool Result distillation archives complete Assistant and autonomous-analysis tool outputs as user/run-scoped gzip artifacts before constructing model context. Deterministic SQL/table, report, Python, error, and JSON reducers retain exact facts, schema, evidence IDs, validation issues, and error locations; large results may additionally use verified small-model batch Map-Reduce. Workflow state keeps only a dynamic bounded projection and the Artifact reference. Raw tool output is short-lived operational data, never LangMem content.
...
migrations/
  versions/
    ...
    0015_memory_convergence.py
    0016_tool_result_distillation.py
    0017_tool_result_map_reduce.py
    0018_tool_result_continuation.py
```
...
- `0015_memory_convergence` adds production recall/audit indexes and Agent/final-validation fields to Memory usage records after LangMem becomes the single long-term-memory path.
- `0016_tool_results` adds short-term tool-result artifacts, versioned summaries, and future Map-Reduce chunk state. Full payloads remain in protected gzip storage; SQLite/PostgreSQL store only ownership, hashes, lifecycle, paths, and summary metadata.
- `0017_tool_map_reduce` persists verified per-chunk summaries, provider/model identity, token usage, and validation issues for resumable and auditable small-model Map-Reduce distillation.
- `0018_tool_continuation` adds idempotent, user/run-scoped, query-focused source projections. It stores only bounded excerpts and audit metadata; raw payloads remain in protected gzip storage and cannot be replayed through model tools.
...
- LangMem Phase A/B established and calibrated the LangGraph `BaseStore` adapter over the existing versioned Memory Repository with strict `(user, scope type, scope, kind)` namespaces, guarded background formation, centralized Agent projections, async Worker compatibility, and content-free comparison metrics.
- LangMem Phase C removes the duplicate Legacy/Shadow recall branches and makes the Store Adapter the only long-term-memory path. SQLite remains the local store, PostgreSQL the Docker/production store, and BGE the local reranker without a separate vector database. `DataMindMemoryGuard`, immutable versions, feedback, reversible dormancy, drift invalidation, per-Agent recall and final-validation audit remain authoritative; SQL/Python cannot search Memory directly.
- LangMem Phase C reverified 2026-08-18: Ruff and targeted Mypy passed; 430 core backend tests plus the migration-contract regression passed; deterministic release and Memory benchmarks passed with `Precision@8=97.5%`, `Recall@8=97.5%`, harmful-memory adoption `0%`, superseded-memory usage `0%`, and 500-memory retrieval P95 `87.4ms`; the frontend production build and all 62 desktop/mobile Playwright cases passed. Alembic `0001 -> 0015`, `0015 -> 0014`, and re-upgrade to `0015` passed on fresh SQLite, the running PostgreSQL stack is at `0015_memory_convergence`, Docker Compose strict readiness passed, and the restarted Celery Worker answered its heartbeat probe.
- Tool Result distillation Phase A introduces a 200MB streaming gzip Artifact Store with hash/idempotency deduplication, 7/30-day lifecycle policies, deterministic domain reducers, Assistant/Analysis execution-boundary integration, and the `tool-context` release benchmark. Complete raw results do not enter LangMem.
- Tool Result distillation Phase B adds thresholded small-model batch Map-Reduce, exact source-quote and numeric-claim verification, idempotent per-chunk persistence, deterministic failure fallback, and dynamic per-artifact context allocation. Small outputs retain the fast deterministic path; large-result distillation cannot bypass the final Context Budget Manager.
- Tool Result distillation Phase C adds query-focused exact continuation with strict user/run isolation, per-artifact call limits, streaming scan limits, idempotent projection caching, and lazy evidence hydration for Python/chart/validation tools. The main model cannot request raw paths or replay complete payloads.
- Tool Result distillation Phase D makes the production Compose context budget enforceable, binds validated report evidence to report retention, removes expired/orphaned artifacts through API lifespan and Celery Beat cleanup, and extends the deterministic benchmark with exact late-value recovery and hard projection bounds.
- Tool Result distillation Phase A reverified 2026-08-18: full Ruff and targeted Mypy passed; 260 Unit, 84 Workflow, and 92 Integration tests passed, followed by 59 affected-path regressions after fail-closed hardening; `release`, `context`, and `tool-context` benchmarks passed; Alembic `0001 -> 0016`, `0016 -> 0015`, and re-upgrade to `0016` passed on fresh SQLite; the frontend production build and Docker Compose configuration passed.
- Tool Result distillation Phase B reverified 2026-08-19: full Ruff and targeted Mypy passed; 264 Unit, 84 Workflow, and 92 Integration tests passed, plus 63 combined Agent Loop/Kimi/Tool Result regressions. `release`, `context`, and `tool-context` v2 benchmarks passed, including unsupported-number rejection. Alembic `0001 -> 0017`, `0017 -> 0016`, and re-upgrade to `0017` passed on fresh SQLite; the frontend production build and Docker Compose configuration passed. A clean `datamind-app:phase-b` image was rebuilt and independently verified for module import and migration execution.
- Tool Result distillation Phase C/D reverified 2026-08-19: full Ruff and targeted Mypy passed; 270 Unit, 84 Workflow, and 92 Integration tests passed, plus 90 combined Tool Result, Agent Loop, Kimi, and analysis regressions. `release`, `memory`, `context`, and `tool-context` v3 benchmarks passed; Tool context reduction was 95.45%, exact-continuation reduction 96.30%, exactness and hard-bound compliance 100%, required-context preservation 100%, and context compression P95 11.05ms. Alembic `0001 -> 0018`, `0018 -> 0017`, and re-upgrade to `0018` passed on fresh SQLite. The frontend production build and all 62 desktop/mobile Playwright cases passed; all Compose images were rebuilt, running PostgreSQL reached `0018_tool_continuation`, and strict readiness reported database, Redis, Worker, Python Runner, MCP Registry, BGE, and Assistant model ready with production context enforcement active.
